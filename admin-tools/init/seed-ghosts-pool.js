...
try {
  ...
  // コピー元（ゴースト/悪魔画像があるフォルダ）
  const localImagesDir = join(__dirname, '../web/public/images/ghosts');
  if (!existsSync(localImagesDir)) {
    console.error(`エラー: ゴースト画像ディレクトリが見つかりません。 ${localImagesDir}`);
    process.exit(1);
  }

  const files = readdirSync(localImagesDir).filter(f => f.match(/^ghost_\d+\.webp$/i));
  console.log(`ゴースト画像ディレクトリーから ${files.length} 件のファイルを検出しました。`);
  ...
  for (const [index, fileName] of files.entries()) {
    ...
    // ドキュメントIDは拡張子を除いたファイル名にする (e.g. ghost_01)
  ...
  console.log('🎉 ゴースト（悪魔）画像のバケット転送および /image_pool へのプール登録がすべて完了しました！');
  console.log('これで本番 Web 上の「画像変更」機能から手動でゴーストを各個別にアタッチできるようになります。');
} catch (error) {
  console.error('ゴーストプール移行中にエラーが発生しました:', error);
  process.exit(1);
}
